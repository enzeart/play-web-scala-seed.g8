...
export const createAppModuleSourceFile = (tree: Tree): ts.SourceFile => {
  ...
  if (!appModuleBuffer) throw new SchematicsException(`Could not find ${FilePaths.APP_MODULE}`);
  return ts.createSourceFile(FilePaths.APP_MODULE, appModuleBuffer.toString('utf-8'), ts.ScriptTarget.Latest, true);
}
